...
fun triggerSwitch(deviceIp: String, pin: Int, password: String, onError: (String) -> Unit): Boolean? {
    val url = "http://$deviceIp/control?pin=$pin&password=$password"

    val responseType = getResponseType<EspButtonTriggerResponseDto>()
    return try {
        val response = Api.getInstance().getSync<EspButtonTriggerResponseDto>(url, responseType)
        response?.on
    } catch (exception: Exception) {
        onError(exception.message ?: "Cannot this trigger switch")
        null
    }
}

fun triggerCalibration(deviceIp: String): List<CalibrationData>? {
    val url = "http://$deviceIp/calibrate"

    val responseType = getResponseType<List<CalibrationData>>()
    return Api.getInstance().postSync<List<CalibrationData>>(url, responseType)
}
